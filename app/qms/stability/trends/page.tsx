...
export default function StabilityTrendsPage() {
  ...
  const withResults = records.filter((r) => !['draft', 'cancelled'].includes(r.status));

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Trend Analysis</h1>
        <p className="text-muted-foreground text-sm">Stability trends synced to CPV Trend Analysis</p>
      </div>
      <Card>
            ...
            <Table>
              ...
              <TableBody>
                {withResults.length === 0 ? (
                  <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No studies with trend data</TableCell></TableRow>
                ) : withResults.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      <Link href={`/qms/stability/${r.id}/trends`}>
