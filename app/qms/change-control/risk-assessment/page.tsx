...
export default function RiskAssessmentQueuePage() {
  const { records, loading } = useChangeControls();
  const filtered = records.filter((r) =>
    r.risk_assessment_required && ['impact_assessment', 'risk_assessment', 'under_qa_review'].includes(r.status),
  );

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Risk Assessment Queue</h1>
        <p className="text-muted-foreground text-sm">Change controls requiring FMEA risk assessment</p>
      </div>
      <Card>
