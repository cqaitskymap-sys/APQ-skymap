...
export default function EffectivenessQueuePage() {
  const { records, loading } = useChangeControls();
  const filtered = records.filter((r) =>
    r.effectiveness_check_required && ['implemented', 'effectiveness_pending'].includes(r.status),
  );

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Effectiveness Review</h1>
        <p className="text-muted-foreground text-sm">Changes pending post-implementation effectiveness verification</p>
      </div>
      <Card>
              ...
              <TableBody>
                {filtered.length === 0 ? (
                  ...
                  <TableRow key={r.id}>
                    ...
                    <TableCell>{r.actual_implementation_date || '—'}</TableCell>
                    <TableCell><CcStatusBadge status={r.status} /></TableCell>
                    <TableCell>
                      <Link href={`/qms/change-control/${r.id}`}>
