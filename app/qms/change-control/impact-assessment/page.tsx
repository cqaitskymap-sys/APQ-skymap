...
export default function ImpactAssessmentQueuePage() {
  const { records, loading } = useChangeControls();
  const filtered = records.filter((r) =>
    ['submitted', 'under_qa_review', 'impact_assessment'].includes(r.status),
  );

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Impact Assessment Queue</h1>
        <p className="text-muted-foreground text-sm">Change controls pending impact assessment review</p>
      </div>
      <Card>
