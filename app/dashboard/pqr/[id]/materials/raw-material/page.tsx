...
export default function RawMaterialReviewPage() {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      // Fetch material reviews for Raw Materials (includes excipient, solvent, preservative, buffer, ph_adjuster)
      const { data: materialsData } = await supabase
        .from('material_review')
        .select('*')
        .eq('pqr_id', id)
        .in('material_type', ['raw_material', 'excipient', 'solvent', 'preservative', 'buffer', 'ph_adjuster'])
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Raw Materials Review</h1>
            <p className="text-sm text-muted-foreground">{pqr.pqr_number} • {pqr.product_name}</p>
          </div>
        </div>
        <Link href={`/dashboard/pqr/${pqr.id}/materials/create?material_type=raw_material`}>
