...
export function FirebaseSetupBanner() {
  if (isDemoAuthEnabled()) {
    return (
      <div className="bg-blue-50 border-b border-blue-200 px-4 py-2 text-sm text-blue-900 flex items-center gap-2">
        ...
        <span>
          Demo auth mode is on (local only). Sign up and login work without Google. Set{' '}
          <code className="bg-blue-100 px-1 rounded">NEXT_PUBLIC_DEMO_AUTH=false</code> when Firebase SSL is fixed.
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="bg-amber-50 border-b border-amber-200 px-4 py-2 text-sm text-amber-900 flex items-center gap-2">
      ...
      <span>
        Firebase is using fallback config. Copy <code className="bg-amber-100 px-1 rounded">.env.local.example</code> to{' '}
