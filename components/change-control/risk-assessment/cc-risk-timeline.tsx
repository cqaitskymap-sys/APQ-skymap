...
import { mapCcRiskAuditToTimeline } from '@/lib/cc-risk-records';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';

export function CcRiskTimeline({ auditLogs }: { auditLogs: Record<string, unknown>[] }) {
  const entries = mapCcRiskAuditToTimeline(auditLogs);
  if (!entries.length) {
    return <p className="text-sm text-muted-foreground py-8 text-center">No risk assessment audit entries yet.</p>;
  }
  return (
    <Card>
