...
import { closureStatusColor } from '@/lib/cc-closure-records';
import { CheckCircle2, Circle } from 'lucide-react';
import type { CcClosureChecklistItem } from '@/lib/cc-closure-records';
import type { CcClosureTimelineEntry } from '@/lib/change-control-types';

export function CcClosureStatusBadge({ status }: { status?: string }) {
  return (
...
export function CcClosureReadinessBar({ percent }: { percent: number }) {
...
export function CcClosureChecklistCard({ item }: { item: CcClosureChecklistItem }) {
  return (
    <div className={cn(
...
export function CcClosureTimeline({ entries }: { entries: CcClosureTimelineEntry[] }) {
