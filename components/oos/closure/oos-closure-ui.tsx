...
import { oosClosureStatusColor } from '@/lib/oos-closure-records';
import { CheckCircle2, Circle } from 'lucide-react';

export function OosClosureStatusBadge({ status }: { status?: string }) {
  const label = status || 'Pending';
  return (
    <span className={cn('inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium', oosClosureStatusColor(label))}>
      {label}
    </span>
  );
}

export function OosClosureReadinessBar({ percent }: { percent: number }) {
...
export function OosClosureChecklistCard({
...
export function OosClosureTimeline({
  entries,
}: {
