...
import type { OosAuditEntry } from '@/lib/oos-audit-trail-records';
import {
  OOS_TIMELINE_SECTIONS,
  formatAuditDateTimeLocal,
  groupTimelineBySection,
} from '@/lib/oos-audit-trail-records';
import { OosAuditActionBadge } from './oos-audit-action-badge';
import { OosAuditUserBadge } from './oos-audit-user-badge';
import { StatusBadge } from '@/components/admin/dashboard/status-badge';

interface OosAuditTimelineProps {
  entries: OosAuditEntry[];
  emptyMessage?: string;
  grouped?: boolean;
}

export function OosAuditTimeline({ entries, emptyMessage, grouped = false }: OosAuditTimelineProps) {
  if (!entries.length) {
    return (
  ...
  if (grouped) {
    ...
    return (
      <div className="space-y-8">
        {OOS_TIMELINE_SECTIONS.map((section) => {
...
function TimelineList({ entries }: { entries: OosAuditEntry[] }) {
  return (
        ...
        <div key={entry.id || entry.audit_id} className="relative pl-4">
          ...
          <div className="rounded-lg border bg-card p-4 shadow-sm">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <OosAuditActionBadge action={entry.action_type} />
            ...
            {entry.oos_number && (
              <p className="text-xs font-mono text-blue-600 mt-1">{entry.oos_number}</p>
            )}
            <div className="mt-2">
              <OosAuditUserBadge name={entry.changed_by_name} role={entry.changed_by_role} />
            </div>
            {entry.field_name && (
