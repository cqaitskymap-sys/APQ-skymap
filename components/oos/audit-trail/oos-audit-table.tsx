...
import { formatAuditDateTimeLocal } from '@/lib/oos-audit-trail-records';
...
interface OosAuditTableProps {
  entries: OosAuditEntry[];
  compact?: boolean;
}

export function OosAuditTable({ entries, compact }: OosAuditTableProps) {
  if (!entries.length) {
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            {!compact && <TableHead>OOS No</TableHead>}
        ...
        <TableBody>
          {entries.map((e) => (
            <TableRow key={e.id || e.audit_id}>
              ...
              {!compact && (
                <TableCell className="text-xs font-mono text-blue-600">{e.oos_number || '—'}</TableCell>
              )}
              <TableCell><OosAuditActionBadge action={e.action_type} /></TableCell>
              ...
              <TableCell><OosAuditUserBadge name={e.changed_by_name} role={e.changed_by_role} /></TableCell>
              {!compact && (
