...
import type { TrainingDashboardCharts } from '@/lib/training-dashboard-types';
...
function safeData<T>(data: T[] | undefined | null): T[] {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <ChartShell title="Pending vs Completed Training" empty={pendingCompleted.every((d) => d.value === 0)}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={pendingCompleted} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label>
              {pendingCompleted.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
            </Pie>
            <Tooltip />
            <Legend />
          </PieChart>
      ...
      <ChartShell title="SOP Training Compliance Trend" empty={sop.length === 0}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={sop}>
            ...
            <YAxis domain={[0, 100]} />
            <Tooltip />
            <Line type="monotone" dataKey="value" stroke="#0891b2" strokeWidth={2} name="Compliance %" />
          </LineChart>
        </ResponsiveContainer>
      </ChartShell>
