...
import {
  RECALL_TIMELINE_SECTIONS,
  formatAuditDateTimeLocal,
  groupRecallTimelineBySection,
...
interface RecallAuditTimelineProps {
  entries: RecallAuditEntry[];
  emptyMessage?: string;
  grouped?: boolean;
}

export function RecallAuditTimeline({ entries, emptyMessage, grouped = false }: RecallAuditTimelineProps) {
  if (!entries.length) {
    return (
  ...
  if (grouped) {
    const groups = groupRecallTimelineBySection(entries);
    return (
      <div className="space-y-8">
        {RECALL_TIMELINE_SECTIONS.map((section) => {
...
function TimelineList({ entries }: { entries: RecallAuditEntry[] }) {
  return (
        ...
        <div key={entry.id || entry.audit_id} className="relative pl-4">
          ...
          <div className="rounded-lg border bg-card p-4 shadow-sm">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <RecallAuditActionBadge action={entry.action_type} />
            ...
            {entry.recall_number && (
              <p className="text-xs font-mono text-blue-600 mt-1">{entry.recall_number}</p>
            )}
            <div className="mt-2">
              <RecallAuditUserBadge name={entry.changed_by_name} role={entry.changed_by_role} />
            </div>
            {entry.field_name && (
