...
import type { SopCharts } from '@/lib/sop-types';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';

const COLORS = ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#ec4899', '#64748b'];

export function SopCharts({ charts }: { charts: SopCharts }) {
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <ChartCard title="SOP Status Distribution">
      ...
      <ChartCard title="Department-wise SOPs">
      ...
      <ChartCard title="Periodic Review Trend">
        {charts.periodicReviewTrend.length > 0 ? (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={charts.periodicReviewTrend}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="month" tick={{ fontSize: 10 }} />
              <YAxis tick={{ fontSize: 10 }} />
              <Tooltip />
