...
import { computeRiskLevel, computeRiskScore, riskLevelColor } from '@/lib/deviation-impact-records';
...
export function RiskCalculator({ severity, occurrence, detection, onChange, disabled }: RiskCalculatorProps) {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-4 rounded-lg bg-muted/50 p-4">
          ...
          <div>
            ...
            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${riskLevelColor(level)}`}>
              {level}
            </span>
          </div>
