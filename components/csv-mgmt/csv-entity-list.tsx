...
interface CsvEntityListProps {
  ...
  onRefresh: () => void;
  renderForm: (props: { systems: CsvSystem[]; onSuccess: () => void; onClose: () => void }) => React.ReactNode;
  exportFn?: () => void;
}

export function CsvEntityList({ title, description, records, loading, columns, onRefresh, renderForm, exportFn }: CsvEntityListProps) {
  const { systems } = useCsvSystems({});
    ...
    <div className="space-y-6">
              ...
              {records.length === 0 ? <TableRow><TableCell colSpan={columns.length + 1} className="text-center py-8 text-muted-foreground">No records</TableCell></TableRow>
                : records.map((r) => (
                  <TableRow key={String(r.id)}>
                    ...
                    <TableCell><Link href={`/qms/csv/${r.system_id}`}><Button variant="ghost" size="sm"><Eye className="h-4 w-4" /></Button></Link></TableCell>
                  </TableRow>
                ))}
            </TableBody></Table>
          </CardContent></Card>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
          ...
          <div className="mt-6">{renderForm({ systems, onSuccess: () => { setOpen(false); onRefresh(); toast.success('Saved'); }, onClose: () => setOpen(false) })}</div>
        </SheetContent>
      </Sheet>
    </div>
  );
