...
import { formatAuditDateTimeLocal } from '@/lib/complaint-audit-trail-records';
...
interface ComplaintAuditTableProps {
  entries: ComplaintAuditEntry[];
  compact?: boolean;
}

export function ComplaintAuditTable({ entries, compact }: ComplaintAuditTableProps) {
  if (!entries.length) {
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            {!compact && <TableHead>Complaint No</TableHead>}
        ...
        <TableBody>
          {entries.map((e) => (
            <TableRow key={e.id || e.audit_id}>
              ...
              {!compact && (
                <TableCell className="text-xs font-mono text-blue-600">{e.complaint_number || '—'}</TableCell>
              )}
              <TableCell><ComplaintAuditActionBadge action={e.action_type} /></TableCell>
              ...
              <TableCell><ComplaintAuditUserBadge name={e.changed_by_name} role={e.changed_by_role} /></TableCell>
              {!compact && (
