...
import type { RiskAuditEntry } from '@/lib/risk-audit-trail-records';
import { formatAuditDateTimeLocal } from '@/lib/risk-audit-trail-records';
import { RiskAuditActionBadge } from './risk-audit-action-badge';
import { RiskAuditUserBadge } from './risk-audit-user-badge';
import {
  Table, TableBody, TableCell, TableHead, TableHeader, TableRow,
} from '@/components/ui/table';
import { StatusBadge } from '@/components/admin/dashboard/status-badge';

interface RiskAuditTableProps {
  entries: RiskAuditEntry[];
  compact?: boolean;
}

export function RiskAuditTable({ entries, compact }: RiskAuditTableProps) {
  if (!entries.length) {
        ...
        <TableBody>
          {entries.map((e) => (
            <TableRow key={e.id || e.audit_id}>
              ...
              {!compact && (
                <TableCell className="text-xs font-mono text-blue-600">{e.risk_number || '—'}</TableCell>
              )}
              <TableCell><RiskAuditActionBadge action={e.action_type} /></TableCell>
              ...
              <TableCell><RiskAuditUserBadge name={e.changed_by_name} role={e.changed_by_role} /></TableCell>
              {!compact && (
