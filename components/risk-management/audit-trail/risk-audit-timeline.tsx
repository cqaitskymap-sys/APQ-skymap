...
export function RiskAuditTimeline({ entries, emptyMessage, grouped = false }: RiskAuditTimelineProps) {
  if (!entries.length) {
    return (
  ...
  if (grouped) {
    ...
    return (
      <div className="space-y-8">
        {RISK_TIMELINE_SECTIONS.map((section) => {
...
function TimelineList({ entries }: { entries: RiskAuditEntry[] }) {
  return (
        ...
        <div key={entry.id || entry.audit_id} className="relative pl-4">
          ...
          <div className="rounded-lg border bg-card p-4 shadow-sm">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <RiskAuditActionBadge action={entry.action_type} />
            ...
            {entry.risk_number && (
              <p className="text-xs font-mono text-blue-600 mt-1">{entry.risk_number}</p>
            )}
            <div className="mt-2">
              <RiskAuditUserBadge name={entry.changed_by_name} role={entry.changed_by_role} />
            </div>
            {entry.field_name && (
