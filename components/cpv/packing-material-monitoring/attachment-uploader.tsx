...
import { PACKING_STORAGE_MODULE } from '@/lib/cpv-packing-material-monitoring';
import type { PackingMaterialAttachment } from '@/lib/cpv-packing-material-monitoring';
...
export function PackingAttachmentUploader({
  recordId,
  uploadedBy,
  attachments,
  onChange,
  ...
  onChange: (files: PackingMaterialAttachment[]) => void;
  disabled?: boolean;
}) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const meta = await uploadFile({
        moduleName: PACKING_STORAGE_MODULE,
        documentId: recordId,
        file,
        fileName: file.name,
        uploadedBy,
      ...
      onChange([...attachments, { ...meta, category }]);
  ...
  const handleDelete = async (file: PackingMaterialAttachment) => {
    try {
      await deleteFile(PACKING_STORAGE_MODULE, recordId, file.fileName);
