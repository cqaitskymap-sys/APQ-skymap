...
import type { CapaCorrectiveActionTimelineEntry } from '@/lib/capa-types';

export function CapaCorrectiveActionTimeline({ entries }: { entries: CapaCorrectiveActionTimelineEntry[] }) {
  if (!entries?.length) {
    return <p className="text-sm text-muted-foreground">No corrective action activity recorded yet.</p>;
  }
  return (
