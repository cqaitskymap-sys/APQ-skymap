...
import type { CapaPreventiveActionTimelineEntry } from '@/lib/capa-types';

export function CapaPreventiveActionTimeline({ entries }: { entries: CapaPreventiveActionTimelineEntry[] }) {
  if (!entries?.length) {
    return <p className="text-sm text-muted-foreground">No preventive action activity recorded yet.</p>;
  }
  return (
