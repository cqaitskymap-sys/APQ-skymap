...
import type { CapaEffectivenessTimelineEntry } from '@/lib/capa-types';

export function CapaEffectivenessTimeline({ entries }: { entries: CapaEffectivenessTimelineEntry[] }) {
  if (!entries?.length) {
    return <p className="text-sm text-muted-foreground">No effectiveness activity recorded yet.</p>;
  }
  return (
    <ol className="relative space-y-4 border-l border-slate-200 pl-4 dark:border-slate-700">
      {entries.map((entry, i) => (
        <li key={`${entry.action}-${entry.at}-${i}`} className="relative">
          <span className="absolute -left-[1.35rem] top-1 h-3 w-3 rounded-full border-2 border-white bg-teal-600 dark:border-slate-900" />
