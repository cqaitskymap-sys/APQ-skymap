...
import {
  ...
  ROLE_MATRIX_MODULES, ROLE_MATRIX_ACTIONS, DATA_SCOPE_OPTIONS, FIELD_ACCESS_LEVELS,
  DEPARTMENT_TYPES, DESIGNATION_LEVELS, EMPLOYMENT_CATEGORIES, JOB_BANDS, JOB_GRADES, REPORTING_LEVELS,
...
const fieldPolicySchema = z.object({
  ...
  access: z.enum(FIELD_ACCESS_LEVELS).default('Editable'),
...
export const roleSchema = z.object({
  ...
  siteAccess: z.string().default(''),
  businessUnitAccess: z.string().default(''),
  dataScope: z.enum(DATA_SCOPE_OPTIONS).default('Organization Records'),
  fieldPolicies: z.array(fieldPolicySchema).default([]),
...
export const roleFormSchema = z.object({
  roleId: z.string().min(1, 'Role ID is required')
    .regex(/^[a-z][a-z0-9_]{1,63}$/, 'Role ID must be lowercase snake_case (2–64 chars)'),
  roleName: z.string().min(1, 'Role name is required').max(120),
  ...
  businessUnitAccess: z.string().default(''),
  dataScope: z.enum(DATA_SCOPE_OPTIONS).default('Organization Records'),
  fieldPolicies: z.array(fieldPolicySchema).max(100).default([]),
  status: z.enum(RECORD_STATUSES).default('Active'),
  permissions: matrixPermissionsSchema,
...
export const departmentSchema = z.object({
  ...
  parentDepartmentId: z.string().default(''),
  parentDepartmentName: z.string().default(''),
  departmentHead: z.string().default(''),
  departmentHeadId: z.string().default(''),
  ...
  isSystemDepartment: z.boolean().optional(),
  isDeleted: z.boolean().optional(),
});
...
export const departmentFormSchema = z.object({
  departmentCode: z.string().min(1, 'Department code is required')
    .max(32)
    .regex(/^[A-Za-z0-9][A-Za-z0-9_-]{0,31}$/, 'Code must be alphanumeric (optional _ or -)'),
  departmentName: z.string().min(1, 'Department name is required').max(160),
  ...
  costCenter: z.string().max(64).default(''),
...
export const designationSchema = z.object({
  ...
  parentDesignationId: z.string().default(''),
  parentDesignationName: z.string().default(''),
  designationLevel: z.enum(DESIGNATION_LEVELS).default('Executive'),
  reportingLevel: z.enum(REPORTING_LEVELS).or(z.literal('')).default(''),
  jobGrade: z.enum(JOB_GRADES).or(z.literal('')).default(''),
  jobBand: z.enum(JOB_BANDS).or(z.literal('')).default(''),
  jobLevel: z.string().default(''),
  employmentCategory: z.enum(EMPLOYMENT_CATEGORIES).default('Permanent'),
  minimumExperience: z.coerce.number().min(0).max(50).default(0),
  requiredQualification: z.string().default(''),
  ...
  description: z.string().default(''),
  remarks: z.string().default(''),
  isSystemDesignation: z.boolean().optional(),
  isDeleted: z.boolean().optional(),
});
...
export const designationFormSchema = z.object({
  designationCode: z.string().min(1, 'Designation code is required')
    .max(32)
    .regex(/^[A-Za-z0-9][A-Za-z0-9_-]{0,31}$/, 'Code must be alphanumeric (optional _ or -)'),
  designationName: z.string().min(1, 'Designation name is required').max(160),
  shortName: z.string().max(40).default(''),
  department: z.string().min(1, 'Department is required'),
  parentDesignationId: z.string().default(''),
  ...
  jobGrade: z.enum(JOB_GRADES).or(z.literal('')).default(''),
  jobBand: z.enum(JOB_BANDS).or(z.literal('')).default(''),
  jobLevel: z.string().max(40).default(''),
  employmentCategory: z.enum(EMPLOYMENT_CATEGORIES).default('Permanent'),
  minimumExperience: z.coerce.number().min(0).max(50).default(0),
  requiredQualification: z.string().max(500).default(''),
