...
import {
  ...
  syncOverdueAssignments, syncDmsTrainingLinks, listTrainingRecords,
} from '@/lib/training-service';
import type { TrainingAssignment, TrainingEffectiveness, TrainingMatrixRow, TmsActor } from '@/lib/training-types';
import { isOverdue, calcCompliance } from '@/lib/training-types';
import {
  emptyTrainingCharts, emptyTrainingKpis, mapAssignmentDashboardStatus,
  classifyTrainingType, type TrainingDashboardData, type TrainingDashboardFilters,
  type TrainingDashboardKpis, type TrainingDashboardCharts,
  type RecentAssignmentRow, type OverdueTrainingRow, type EffectivenessPendingRow,
  type TrainingActivityEntry, TRAINING_DASHBOARD_MODULE,
} from '@/lib/training-dashboard-records';

export type TrainingDashboardActor = TmsActor;
...
function filterAssignments(
  ...
  if (filters?.training_type) rows = rows.filter((a) => classifyTrainingType(a) === filters.training_type || a.training_type === filters.training_type);
...
function computeKpis(
  ...
  const pending = assignments.filter((a) => ['pending', 'in_progress', 'retraining'].includes(a.status)).length;
  const overdue = assignments.filter((a) => a.status === 'overdue' || isOverdue(a.due_date, a.status)).length;

  const weekEnd = new Date();
  weekEnd.setDate(weekEnd.getDate() + 7);
  const weekEndStr = weekEnd.toISOString().split('T')[0];
  const dueThisWeek = assignments.filter(
    (a) => !['completed', 'cancelled'].includes(a.status) && a.due_date >= today() && a.due_date <= weekEndStr,
  ).length;

  const effPending = assignments.filter((a) => {
    const st = mapAssignmentDashboardStatus(a, effectiveness);
    return st === 'Effectiveness Pending';
  }).length;
  ...
  return {
    ...
    sopTrainings: assignments.filter((a) => classifyTrainingType(a) === 'SOP Training').length,
    inductionTrainings: assignments.filter((a) => classifyTrainingType(a) === 'Induction').length,
    refresherTrainings: assignments.filter((a) => classifyTrainingType(a) === 'Refresher Training').length,
    ...
    usersNotTrained: usersWithPending.size,
...
function computeCharts(
  ...
  for (const a of assignments) {
    ...
    if (a.status === 'overdue' || isOverdue(a.due_date, a.status)) {
      const m = a.due_date.slice(0, 7);
      overdueTrend[m] = (overdueTrend[m] || 0) + 1;
    }

    if (type === 'SOP Training') {
      const m = (a.assigned_date || today()).slice(0, 7);
      if (!sopTrend[m]) sopTrend[m] = { total: 0, completed: 0 };
      sopTrend[m].total++;
      if (a.status === 'completed') sopTrend[m].completed++;
    }

    if (!userStatus[a.employee_name]) userStatus[a.employee_name] = { completed: 0, pending: 0, overdue: 0 };
    if (a.status === 'completed') userStatus[a.employee_name].completed++;
    else if (a.status === 'overdue' || isOverdue(a.due_date, a.status)) userStatus[a.employee_name].overdue++;
  ...
  const pending = assignments.filter((a) => !['completed', 'cancelled'].includes(a.status)).length;
...
function buildTables(
  ...
  const recent = [...assignments]
    ...
    .map((a) => ({
      ...
      document_sop: a.document_number || a.sop_version ? `${a.document_number || ''} ${a.sop_version || ''}`.trim() : '—',
      due_date: a.due_date,
...
export async function fetchTrainingDashboard(
  ...
  try {
    if (actor?.id) {
      ...
      await syncDmsTrainingLinks(actor);
    }
    ...
    await listTrainingRecords().catch(() => []);

    const assignments = filterAssignments(assignmentsRaw, filters);
    const scopedMatrix = filters?.department
      ? matrix.filter((m) => m.department === filters.department)
      : matrix;

    const kpis = computeKpis(assignments, effectiveness, scopedMatrix, employees.length);
...
export async function refreshTrainingDashboard(
  ...
  if (actor) {
    await logAuditEvent({
      userId: actor.id,
      userName: actor.name,
      module: TRAINING_DASHBOARD_MODULE,
      recordId: 'training-dashboard',
      action: 'DASHBOARD_REFRESH',
      oldValue: '',
      newValue: JSON.stringify({ filters, kpis: data.kpis }),
      reason: 'Training dashboard refreshed',
      ipAddress: 'client',
...
export async function logTrainingDashboardExport(
  ...
  await logAuditEvent({
    ...
    oldValue: '',
    newValue: format,
    reason: `Training dashboard ${format} export requested`,
    ipAddress: 'client',
    device: typeof navigator !== 'undefined' ? navigator.userAgent : 'server',
    status: 'Success',
  });
...
export async function logTrainingDashboardViewed(actor: TrainingDashboardActor): Promise<void> {
  await logAuditEvent({
    ...
    oldValue: '',
    newValue: 'view',
    reason: 'Training management dashboard viewed',
    ipAddress: 'client',
    device: typeof navigator !== 'undefined' ? navigator.userAgent : 'server',
    status: 'Success',
  });
...
export function openTrainingDashboardPdfPlaceholder(
  ...
  const html = `<!DOCTYPE html><html><head><title>Training Dashboard Export</title>
    <style>body{font-family:Arial,sans-serif;padding:24px}table{width:100%;border-collapse:collapse;font-size:12px}
    th,td{border:1px solid #ccc;padding:6px;text-align:left}th{background:#2563eb;color:#fff}</style></head>
    <body><h1>Training Management Dashboard</h1>
    <p>Generated by ${generatedBy} on ${new Date().toLocaleString()}</p>
    <h2>KPI Summary</h2>
    <ul>
      <li>Total Trainings: ${data.kpis.totalTrainings}</li>
      <li>Completed: ${data.kpis.completedTrainings}</li>
      <li>Pending: ${data.kpis.pendingTrainings}</li>
      <li>Overdue: ${data.kpis.overdueTrainings}</li>
      <li>Compliance: ${data.kpis.trainingCompliancePercent}%</li>
    </ul>
    <h2>Recent Assignments</h2>
    <table><thead><tr><th>#</th><th>Training No</th><th>Employee</th><th>Department</th><th>Type</th><th>Due Date</th><th>Status</th></tr></thead>
    <tbody>${rows || '<tr><td colspan="7">No records</td></tr>'}</tbody></table>
    <p><em>PDF export placeholder — integrate formal report template in production.</em></p>
    </body></html>`;
